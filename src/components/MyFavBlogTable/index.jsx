...
const MyFavBlogTable = ({
  ...
  // تعریف ستون‌ها برای اخبار
  const columnConfigs = [
    {
      title: 'تصویر',
      accessors: ['currentImageAddressTumb'],
      render: (value) => (
        <img
          src={value || '/src/assets/img/figma.jpg'}
          className="h-12 w-12 rounded object-cover"
          alt={value ? 'تصویر خبر' : 'تصویر پیش‌فرض'}
        />
      ),
    },
    {
    ...
    {
      title: 'امتیاز',
      accessors: ['currentRate'],
      defaultValue: 'بدون امتیاز',
    },
    {
      title: 'تعداد بازدید',
      accessors: ['currentView'],
      defaultValue: '0',
    },
    {
      title: 'تعداد لایک',
      accessors: ['currentLikeCount'],
      defaultValue: '0',
    },
    {
      title: 'تاریخ به‌روزرسانی',
      accessors: ['updateDate'],
      render: (value) =>
    ...
    },
    {
      ...
      render: (value, row) => (
        <div className="flex justify-center gap-2">
          {' '}
          <Link to={`/blogs/single/${row.newsId}`} className="h-6 w-6">
            <IconSet
  ...
  const shortColumns = fullColumns.filter((col) =>
    ['تصویر', 'عنوان خبر', 'عملیات'].includes(col.title),
  );
  ...
  return (
    <div
      className={clsx('mx-4 overflow-hidden rounded-3xl shadow-sm lg:mx-16', {
        'bg-[#292a2d]': darkMode,
          ...
          >
            {displayData?.length > 0 ? (
              displayData.map((row, rowIndex) => (
                <div
                  key={rowIndex}
                  className={clsx('mb-2 flex rounded-lg', {
                  ...
                  {columns.map((column, colIndex) => {
                    ...
                    return (
                      <div
                        key={colIndex}
                        className={clsx(
                          'flex flex-1 items-center justify-center truncate px-4 py-4 text-sm font-bold',
                          {
                            'text-white': darkMode,
              ...
              <div className="flex h-full items-center justify-center">
                <div className="text-center">
                  ...
                  >
                    {query
                      ? 'با معیارهای جستجوی شما هیچ خبری مطابقت ندارد'
                      : 'در حال حاضر هیچ خبری برای نمایش وجود ندارد'}
                  </p>
                </div>
              </div>
            )}
